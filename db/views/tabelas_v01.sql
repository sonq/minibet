...
select users.id AS UserCode,
       ...
       CASE WHEN bettype = 'Score' AND bettype <> 'Result' AND bets.homescore = fixtures.homescore AND bets.awayscore = fixtures.awayscore  THEN 3
            WHEN bettype = 'Result' AND bettype <> 'Score' AND bets.result = fixtures.result THEN 1
            ELSE 0
       END AS Points
from bets
join users
